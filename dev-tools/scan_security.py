...
def run_command(
    ...
    try:
        ...
        error_msg = (
            f"Error: Command not found. Please ensure the tool is installed.\n   Command: {cmd[0]}"
        )
        if not quiet:
            print(f"\n✗ {error_msg}")
...
def run_trivy_fs(
    ...
    return run_command(
        cmd, "Trivy Filesystem Scan", check=False, output_file=output_file, quiet=quiet
    )
...
def run_trivy_image(
    ...
    return run_command(
        cmd, f"Trivy Image Scan: {image_name}", check=False, output_file=output_file, quiet=quiet
    )


def run_opengrep(output_file: Optional[str] = None, quiet: bool = False) -> Tuple[bool, str]:
...
Examples:
  ...
  python dev-tools/scan_security.py --trivy-fs --output trivy-fs-report.txt
    ...
    parser.add_argument("--trivy-fs", action="store_true", help="Run Trivy filesystem scan")
    parser.add_argument(
        "--trivy-image", metavar="IMAGE", help="Run Trivy image scan on specified image"
    )
    parser.add_argument("--opengrep", action="store_true", help="Run OpenGrep secret scan")
    parser.add_argument(
        "--all", action="store_true", help="Run all scans (filesystem, image, and opengrep)"
    )
    parser.add_argument(
        "--trivyignore",
        default=".trivyignore",
    ...
    fs_output = (
        args.output if args.output and run_fs and not (run_image or run_opengrep_scan) else None
    )
    image_output = (
        args.output if args.output and run_image and not (run_fs or run_opengrep_scan) else None
    )
    opengrep_output = (
        args.output if args.output and run_opengrep_scan and not (run_fs or run_image) else None
    )
    ...
    if output_dir:
        ...
        if not image_output and run_image:
            ext = "json" if args.json else "txt"
            image_name_safe = (
                (args.trivy_image or "credshare-build").replace(":", "_").replace("/", "_")
            )
    ...
    if run_fs:
        success, _ = run_trivy_fs(
            args.trivyignore, output_file=fs_output, json_output=args.json, quiet=args.quiet
        )
    ...
    if run_opengrep_scan:
        success, _ = run_opengrep(output_file=opengrep_output, quiet=args.quiet)
